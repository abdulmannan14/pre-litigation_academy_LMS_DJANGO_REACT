...
export default function Navbar() {
  ...
  return (
    <nav className="bg-surface border-b border-divider sticky top-0 z-50">
        ...
        <div className="flex items-center justify-between h-16">
                ...
                <Link
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm transition-colors ${
                    isActive(to)
                      ...
                      : 'text-gray-400 hover:text-secondary hover:bg-accent/50'
                  }`}
                >
                  {label}
                </Link>
          ...
          {user && (
            <div className="flex items-center gap-3">
              ...
              <div className="relative hidden sm:block">
                ...
                {menuOpen && (
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-48 bg-surface rounded-xl shadow-lg border border-divider py-1 z-50">
                      ...
                      {!user.is_staff && (
                        <Link
                          ...
                          className="flex items-center gap-2 w-full text-left px-4 py-2 text-sm text-gray-400 hover:bg-background transition-colors"
                        >
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-red-950/30 transition-colors"
                      >
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <button
                onClick={() => setMenuOpen((o) => !o)}
                className="sm:hidden p-2 rounded-lg text-gray-400 hover:bg-background transition-colors"
              >
                {menuOpen ? (
      ...
      {/* Mobile menu drawer */}
      {user && menuOpen && (
        <div className="sm:hidden border-t border-divider bg-surface px-4 py-3 space-y-1">
          <div className="flex items-center gap-3 px-2 py-2 mb-2 border-b border-divider">
          ...
          {navLinks.map(({ label, to }) => (
            <Link
              ...
              className={`block px-3 py-2 rounded-lg text-sm transition-colors ${
                isActive(to)
                  ? 'text-secondary font-medium bg-accent'
                  : 'text-gray-400 hover:bg-background'
              }`}
            >
              {label}
            </Link>
          ...
          {!user.is_staff && (
            <Link
              to="/profile"
              onClick={() => setMenuOpen(false)}
              className="block px-3 py-2 rounded-lg text-sm text-gray-400 hover:bg-background transition-colors"
            >
              Profile
            </Link>
          )}
          ...
          <button
            onClick={() => { setMenuOpen(false); handleLogout(); }}
            className="w-full text-left px-3 py-2 rounded-lg text-sm text-red-500 hover:bg-red-950/30 transition-colors mt-1"
          >
            Sign out
          </button>
        </div>
